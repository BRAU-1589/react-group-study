...
Todolist 컴포넌트의 부모 컴포넌트인 App 컴포넌트가 리렌더링되는 이유가 
todos 배열이 업데이트될 때이기 때문입니다. 

그래서 지금 TodoList 컴포넌트는 불필요한 리렌더링이 발생하지 않습니다. 
그러나 App 컴포넌트에서 state가 추가되어 해당 값이 업데이트될 때 
