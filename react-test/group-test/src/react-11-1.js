...
지금 상황을 분석하면, 항목을 체크할 경우 BeSlow 컴포넌트의 state가 변경되면서 BeSlow 컴포넌트가 리렌더링된다.
 부모 컴포넌트가 리렌더링되었으니 자식 컴포넌트인 TodoList 컴포넌트가 리렌더링되고,
 그 안의 모든 컴포넌트들이 리렌더링된 것이다.
 하나의 항목을 체크한 것인데 나머지 2499개의 항목이 모두 리렌더링되었기 때문에 애플리케이션에 지연이 발생. 이럴 때 컴포넌트 리렌더링 성능을 최적화하는 작업을 해야한다.
...
함수형 컴포넌트에서는
 라이프사이클 메서드를 사용할 수 없기 때문에 React.memo라는 함수를 사용합니다.
